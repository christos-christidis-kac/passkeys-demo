package com.christidischristidis.passkeys.screen.email
...
import com.christidischristidis.passkeys.screen.util.ScreenUtil.showToast
import com.christidischristidis.passkeys.ui.theme.PasskeysTheme
import com.google.android.gms.fido.Fido
import com.christidischristidis.passkeys.screen.email.EnterEmailViewModel.UserInteraction as Event
...
fun EnterEmailScreen(
    ...
    LaunchedEffect(uiState.requestOptions) {
        uiState.requestOptions?.let { options ->
            val fidoClient = Fido.getFido2ApiClient(context)
            fidoClient.getSignPendingIntent(options)
                .addOnFailureListener {
                    showToast(context, "authentication passkey failed")
                }
...
private fun EnterEmailScreenContent(
    ...
    Column(
        ...
        Button(
            ...
            Text(text = stringResource(R.string.sign_in_with_passkey))
        }
        Spacer(Modifier.height(16.dp))
