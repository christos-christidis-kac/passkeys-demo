...
class EnterEmailViewModel @Inject constructor(
        ...
        viewModelScope.launch {
            when (val apiResult = userRepository.getUserDetailsByEmail(uiState.value.email)) {
                ...
                is ApiResult.Success -> {
                    if (withPasskey) {
                        initWebauthn(apiResult.data.userId)
                    } else {
                        initPasscodeLogin(
    ...
    private suspend fun initWebauthn(userId: String) {
        when (val apiResult = webauthnRepository.initWebauthnLogin(userId)) {
            ...
            is ApiResult.Success -> {
                ...
                updateUiState { it.copy(requestOptions = apiResult.data) }
            }
    ...
    data class UiState(
        ...
        val requestOptions: PublicKeyCredentialRequestOptions? = null,
